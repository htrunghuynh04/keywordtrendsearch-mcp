...
def _client(query: str, lang: str = "en", country: str = "", domain: str = "google.com") -> SeoKeywordResearch:
    return SeoKeywordResearch(query=query, api_key=SERPAPI_KEY, lang=lang, country=country, domain=domain)


@mcp.tool()
def get_autocomplete(query: str, lang: str = "en", country: str = "") -> list:
    """Get Google Autocomplete keyword suggestions for a search query. country: ISO code e.g. 'vn', 'us', 'gb' (empty = global)."""
    return _client(query, lang, country).get_auto_complete()


@mcp.tool()
def get_related_searches(query: str, lang: str = "en", country: str = "", domain: str = "google.com") -> list:
    """Get Google Related Searches for a query. country: ISO code e.g. 'vn', 'us' (empty = global)."""
    return _client(query, lang, country, domain).get_related_searches()


@mcp.tool()
def get_related_questions(query: str, depth_limit: int = 0, lang: str = "en", country: str = "", domain: str = "google.com") -> list:
    """Get People Also Ask questions for a query. depth_limit 0-4 controls how deep to fetch nested questions. country: ISO code e.g. 'vn', 'us' (empty = global)."""
    return _client(query, lang, country, domain).get_related_questions(depth_limit)


@mcp.tool()
def select_target_keywords(query: str, depth_limit: int = 1, lang: str = "en", country: str = "", domain: str = "google.com") -> dict:
    """
...
def get_keyword_trends(
    ...
    country: ISO country code e.g. 'VN', 'US', 'GB' (empty = worldwide)
    """
...
app = mcp.streamable_http_app()  # streamable-http at /mcp
